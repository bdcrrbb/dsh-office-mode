...
~/office-toolchain/venv/bin/python --with 'markitdown[pptx]' python -m markitdown path-to-file.pptx
```
...
#### Unpacking a file
`~/office-toolchain/venv/bin/python ooxml/scripts/unpack.py <office_file> <output_dir>`
...
4. **Visual validation**: Generate thumbnails and inspect for layout issues
   - Create thumbnail grid: `~/office-toolchain/venv/bin/python scripts/thumbnail.py output.pptx workspace/thumbnails --cols 4`
...
2. Unpack the presentation: `~/office-toolchain/venv/bin/python ooxml/scripts/unpack.py <office_file> <output_dir>`
...
1. **Extract template text AND create visual thumbnail grid**:
   * Extract text: `~/office-toolchain/venv/bin/python --with 'markitdown[pptx]' python -m markitdown template.pptx > template-content.md`
   * Read `template-content.md`: Read the entire file to understand the contents of the template presentation. **NEVER set any range limits when reading this file.**
   * Create thumbnail grids: `~/office-toolchain/venv/bin/python scripts/thumbnail.py template.pptx`
...
4. **Duplicate, reorder, and delete slides using `rearrange.py`**:
   * Use the `scripts/rearrange.py` script to create a new presentation with slides in the desired order:
     ```bash
     ~/office-toolchain/venv/bin/python scripts/rearrange.py template.pptx working.pptx 0,34,34,50,52
     ```
...
5. **Extract ALL text using the `inventory.py` script**:
   * **Run inventory extraction**:
     ```bash
     ~/office-toolchain/venv/bin/python scripts/inventory.py working.pptx text-inventory.json
     ```
...
7. **Apply replacements using the `replace.py` script**
   ```bash
   ~/office-toolchain/venv/bin/python scripts/replace.py working.pptx replacement-text.json output.pptx
   ```
...
```bash
~/office-toolchain/venv/bin/python scripts/thumbnail.py template.pptx [output_prefix]
```

**Features**:
- Creates: `thumbnails.jpg` (or `thumbnails-1.jpg`, `thumbnails-2.jpg`, etc. for large decks)
- Default: 5 columns, max 30 slides per grid (5×6)
- Custom prefix: `~/office-toolchain/venv/bin/python scripts/thumbnail.py template.pptx my-grid`
...
```bash
# Basic usage
~/office-toolchain/venv/bin/python scripts/thumbnail.py presentation.pptx

# Combine options: custom name, columns
~/office-toolchain/venv/bin/python scripts/thumbnail.py template.pptx analysis --cols 4
```
