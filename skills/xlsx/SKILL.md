...
5. **Recalculate formulas (MANDATORY IF USING FORMULAS)**: Use the recalc.py script
   ```bash
   ~/office-toolchain/venv/bin/python recalc.py output.xlsx
   ```
...
```bash
~/office-toolchain/venv/bin/python recalc.py <excel_file> [timeout_seconds]
```

Example:
```bash
~/office-toolchain/venv/bin/python recalc.py output.xlsx 30
```
